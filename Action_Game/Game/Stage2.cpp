...
{
	//レベルの構築
	m_level.Init(L"level/myStage_02.tkl", [&](LevelObjectData& objData) {
		if (objData.EqualObjectName(L"unityChan") == true) {
			m_player = NewGO<Player>(0, "Player");
			m_player->SetPosition(objData.position);
			return true;
		}
		else if (objData.EqualObjectName(L"MoveBoard") == true) {
			auto m_moveBoard = NewGO<MoveBoard>(0, "MoveBoard");
			m_moveBoard->SetPosition(objData.position);
			m_moveBoard->SetRotation(objData.rotation);
			m_moveBoard->SetSpeed(200.0f);
			return true;
		}
		...
		else if (objData.EqualObjectName(L"Stage02Ground") == true) {
			m_ground = NewGO<prefab::CSkinModelRender>(0, "ground");
			m_ground->Init(L"modelData/Stage02Ground.cmo");
